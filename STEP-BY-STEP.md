...
Then it asks for the password. The default is this.

```
makerbase
```

> **While you type the password, nothing appears on screen.** No asterisks, no dots. It looks like
...
> **The rule you cannot break.** The middle part must be **exactly** the firmware you noted in
...
> **Deleting `c_helper.so` first is mandatory.** If that file does not exist when the printer
...
Log back in (`ssh mks@YOUR_IP`) and run this.
...
> **Rolling back restores the factory `printer.cfg`.** Every bit of calibration you did after the
...
Happens when the printer was reflashed and its "identity" changed. On your computer run this.
...
> The Elegoo update **can erase your `printer.cfg`** and may force leveling into Standard mode
...
- [@SandMMakers video](https://www.youtube.com/watch?v=Aoy3sI1lv1g)
